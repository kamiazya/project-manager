import { Ticket, TicketId, TicketValidationError } from '@project-manager/domain'
      ...
      it('should handle all valid status values', async () => {
        ...
        for (const testCase of testCases) {
          const ticket = Ticket.create({
            ...
            description: 'Test Description',
            priority: 'medium',
            status: testCase.from as 'pending' | 'in_progress' | 'completed' | 'archived',
          })
          vi.mocked(mockTicketRepository.findById).mockResolvedValue(ticket)

          const request = new UpdateTicketStatus.Request(
            validTicketId,
            testCase.to as 'pending' | 'in_progress' | 'completed' | 'archived'
          )
          const response = await updateTicketStatusUseCase.execute(request)
    ...
    describe('Status Transition Validation', () => {
      ...
      it('should handle complex transition scenarios', async () => {
        ...
        for (const transition of transitions) {
          ...
          const request = new UpdateTicketStatus.Request(
            validTicketId,
            transition.to as 'pending' | 'in_progress' | 'completed' | 'archived'
          )
