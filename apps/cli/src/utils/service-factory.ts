...
import { getStoragePath } from './config.ts' // Singleton factory instance
...
function getUseCaseFactory(): UseCaseFactory {
  if (!factoryInstance) {
    ...
    const jsonRepository = new JsonTicketRepository(getStoragePath())
    const ticketRepository = new CliTicketRepository(jsonRepository)
